...
t = np.array([data[data[:, 1] == p**2][:, 2] for p in ps])
s = np.array([data[data[:, 1] == p**2][:, 3] for p in ps])
e = np.array([data[data[:, 1] == p**2][:, 4] for p in ps])
plot(Ns, t, r'$N$', r'$T$', labels_ps, join(images, "T"))
plot(Ns, s, r'$N$', r'$S$', labels_ps, join(images, "S"))
plot(Ns, e, r'$N$', r'$E$', labels_ps, join(images, "E"))
