...
def fitness_function(individual, n2p):
    distance = 0
    full_way = [0] + individual + [0]
...
def roulette(population, estimates, nindividuals, offset=1):
    ...
    for estimate in normalized_estimates:
        probabilities.append(estimate / normalized_estimates_sum)
    ...
    while len(individuals) != nindividuals:
        ...
        for i, cum_prob in enumerate(cumulative_probabilities):
            if rand_num <= cum_prob:
                ...
                if len(individuals) % 2 and individual == individuals[-1]:
                    break
...
def GA(population_size, epochs, mutation_probability, npoints=30, radius=5):
    best_estimates = []
    average_estimates = []
    worst_estimates = []
    best_estimate = None
    ...
    points = get_dots(npoints, radius)
    n2p, p2n = encode(points)
    ...
    estimates = []
    for individual in population:
        estimates.append(fitness_function(individual, n2p))

    best_estimate = -max(estimates)
    best_individual = population[estimates.index(max(estimates))]
    best_epoch = -1
    ...
    for epoch in range(epochs):
        ...
        for i in range(0, len(crossover_individuals), 2):
            ind1, ind2 = crossover_individuals[i], crossover_individuals[i+1]
        ...
        for individual in new_population:
            new_estimates.append(fitness_function(individual, n2p))
        
        population = new_population
        estimates = new_estimates

        if -max(estimates) < best_estimate:
            best_estimate = -max(estimates)
            best_individual = population[estimates.index(max(estimates))]
            best_epoch = epoch

        print(f'epoch {epoch} best estimate = {-max(estimates)}')
...
if __name__ == '__main__':
    ...
    best_estimates, average_estimates, worst_estimates, best_estimate, best_individual, best_epoch, n2p = GA(populations_size, nepochs, mutation_probability)
