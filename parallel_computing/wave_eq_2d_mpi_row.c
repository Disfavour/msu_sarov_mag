...
int main(int argc, char** argv)
{
	...
	//printf("%d/%d\t%d\ti %d-%d\tio %d-%d\t %d %d\n", rank, size, rank, i_min, i_max, i_min_with_overlap, i_max_with_overlap, i_size, i_size_with_overlap);
	...
	while (t < T - tau/2)
	{
		...
		//printf("%d/%d %d %d %d %d %d\n", rank, size, i_min_with_overlap-i_min_with_overlap, i_max - i_min_with_overlap, i_min - i_min_with_overlap, i_max-1 - i_min_with_overlap, Mx+1);

		MPI_Irecv(A[2][i_min_with_overlap-i_min_with_overlap], Mx+1, MPI_DOUBLE, top, TAG_TOP_TO_BOT, MPI_COMM_WORLD, &request[0]);
		MPI_Isend(A[2][i_max-1 - i_min_with_overlap], Mx+1, MPI_DOUBLE, bot, TAG_TOP_TO_BOT, MPI_COMM_WORLD, &request[1]);

		MPI_Irecv(A[2][i_max - i_min_with_overlap], Mx+1, MPI_DOUBLE, bot, TAG_BOT_TO_TOP, MPI_COMM_WORLD, &request[2]);
		MPI_Isend(A[2][i_min - i_min_with_overlap], Mx+1, MPI_DOUBLE, top, TAG_BOT_TO_TOP, MPI_COMM_WORLD, &request[3]);
		...
		// 	if (rank % 2 == i)
		// 	{
		// 		MPI_Send(A[2][i_min - i_min_with_overlap], Mx+1, MPI_DOUBLE, bot, TAG_TOP_TO_BOT, MPI_COMM_WORLD);
		// 		MPI_Send(A[2][i_max-1 - i_min_with_overlap], Mx+1, MPI_DOUBLE, top, TAG_BOT_TO_TOP, MPI_COMM_WORLD);
		// 	}
		// 	else
		// 	{
		// 		MPI_Recv(A[2][i_max - i_min_with_overlap], Mx+1, MPI_DOUBLE, top, TAG_TOP_TO_BOT, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
		// 		MPI_Recv(A[2][i_min_with_overlap - i_min_with_overlap], Mx+1, MPI_DOUBLE, bot, TAG_BOT_TO_TOP, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
		// 	}
		// }
