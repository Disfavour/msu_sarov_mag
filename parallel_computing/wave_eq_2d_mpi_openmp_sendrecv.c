...
int main(int argc, char** argv)
{
    ...
    #pragma omp parallel
    {
        ...
        while (t < T - tau_2)
        {
            ...
            #pragma omp single
            {
                MPI_Sendrecv(&A[2][il(i_min)][jl(j_max-1)], 1, column, right, LEFT_TO_RIGHT,
                    &A[2][il(i_min)][jl(j_min_overlap)], 1, column, left, LEFT_TO_RIGHT, MPI_COMM_WORLD, MPI_STATUS_IGNORE);

                MPI_Sendrecv(&A[2][il(i_min)][jl(j_min)], 1, column, left, RIGHT_TO_LEFT,
                    &A[2][il(i_min)][jl(j_max_overlap-1)], 1, column, right, RIGHT_TO_LEFT, MPI_COMM_WORLD, MPI_STATUS_IGNORE);

                MPI_Sendrecv(&A[2][il(i_max-1)][jl(j_min)], 1, row, bot, TOP_TO_BOT,
                    &A[2][il(i_min_overlap)][jl(j_min)], 1, row, top, TOP_TO_BOT, MPI_COMM_WORLD, MPI_STATUS_IGNORE);

                MPI_Sendrecv(&A[2][il(i_min)][jl(j_min)], 1, row, top, BOT_TO_TOP,
                    &A[2][il(i_max_overlap-1)][jl(j_min)], 1, row, bot, BOT_TO_TOP, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
